...
namespace CarRace
{
    class Player
    {
        ...
        public Player()
        {
            ...
            {
                color = Color.Blue;
                pos = new Position(0f, 52);  
            }
            if (carColor == 2)
            {
                color = Color.Red;
                pos = new Position(1.5f, 52);  
            }
            if (carColor == 3)
            {
                color = Color.Black;
                pos = new Position(3f, 52);
            }
             

            //carColor++;
            speed = 0.2f + (float)(Prog.Attention()/100) / 18f;
            
            //speed = 0.2f + (float).5 / 18f;
            //speed = 0.2f + (float).25 / 18f;
            //MainForm.Update9(Prog.Attention());
            //speed = (float)Prog.Attention();
        }
        ...
        public void Draw()
        {
            ...
            if (Controller.StartedRace == true)
            {
                ...
                {
                    counter = 0;
                    //speed = 0.2f + (float)(Prog.Attention()/100) / 20f;
                    speed = 0.2f + (float).5 / 20f;
                    //speed = 0.2f + (float).3 / 20f;
                    //speed = (float)Prog.Attention();
                }
            }
